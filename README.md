...
- Exports an HTML result page with:
  ...
  - two backup options for each stage by default
  - per-person travel times
  - stage-to-stage transfer mode and time
  - rating, budget, tags, and reviews when available from AMap data
  - Playful Geometric map markers, legend, and colored lines connecting multi-stop locations
...
The script prints the selected route for Plan A, Plan B, and Plan C, then writes an HTML report.
...
`--option-topn` defaults to `2`, so each stage shows two backup candidates unless overridden.
...
- Plan C usually favors rating, budget, vibe, and semantic match more strongly.
- For multi-stop plans, commute fairness is evaluated for the first stop only; later stops prioritize transfer efficiency and fit.
- Stage-to-stage transfer prefers walking when practical, then transit/subway-style routing when walking is too long.
